...
class CNN_DiffusionUnet(nn.Module):
    ...
    def __init__(self,config, MTS_args):
        ...
        diff_steps=config.diffusion.timesteps
        self.num_vars = MTS_args.c_out
        self.ori_seq_len =MTS_args.seq_len
        self.seq_len = MTS_args.seq_len+MTS_args.pred_len
        self.label_len = MTS_args.label_len
        self.pred_len = MTS_args.pred_len

        kernel_size = 3
        padding = 1
        self.channels = MTS_args.ddpm_inp_embed
        self.lin0 = nn.Linear(12 * MTS_args.d_model_c, MTS_args.seq_len+MTS_args.pred_len)
        self.input_projection = InputConvNetwork(MTS_args, self.num_vars, self.channels, num_layers=MTS_args.ddpm_layers_inp)

        self.dim_diff_steps = MTS_args.ddpm_dim_diff_steps
        ...
        kernel_size = 3
        padding = 1
        self.dim_intermediate_enc = MTS_args.ddpm_channels_fusion_I

        self.enc_conv = InputConvNetwork(MTS_args, self.channels+self.dim_diff_steps, self.dim_intermediate_enc, num_layers=MTS_args.ddpm_layers_I)
        ...
        self.cnn_cond_projections = InputConvNetwork(MTS_args, self.num_vars, self.pred_len, num_layers=MTS_args.cond_ddpm_num_layers, ddpm_channels_conv=MTS_args.cond_ddpm_channels_conv)
        self.cnn_linear = nn.Linear(self.ori_seq_len, self.num_vars)


        self.combine_conv = InputConvNetwork(MTS_args, self.dim_intermediate_enc+self.num_vars+self.num_vars, self.num_vars, num_layers=MTS_args.ddpm_layers_II)


    def forward(self,  x, t, y):
        # yn torch.Size([128, 7, 192])
        # t torch.Size([128])
        # cond_info torch.Size([128, 7, 288])
        x=x.permute(0,2,1)  #[batch, pred_len,var]
        diffusion_step=t
        #y:[batch, nvar, patch_num, d_model_c
        batch, nvar, patch_num, d_model_c= y.shape
        y=y.reshape(batch, nvar, patch_num * d_model_c)
        y=self.lin0(y)
        cond_info=y
        ...
        pred_out = torch.zeros([x.size(0),self.num_vars,self.pred_len],dtype=x.dtype).to(x.device)
        for i in range(self.num_vars):
